...
tools: ["read", "search", supabase-hsnyk-xboost/*]
---
...
## Supabase MCP（必須）
Supabaseのスキーマ操作・型生成・データ確認には **Supabase CLI ではなく Supabase MCP Server** を使用する。
...
- ✅ Always do: Use **Supabase MCP Server** for schema/type/migration operations (not Supabase CLI).
...
- 🚫 Never do: Run `npx supabase` or `supabase` CLI directly (use MCP instead).
